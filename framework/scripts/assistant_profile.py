...
def _activity_themes(entries: list[str]) -> dict[str, int]:
    ...
    scores = {theme: 0 for theme in keyword_groups}
    for item in entries:
        text = item.lower()
...
def main() -> None:
    ...
    print(f"Alias: {profile.get('alias', 'Assistant')}")
    print(f"Ship Mind: {profile.get('ship_mind_name', NAME_CANDIDATES[0])}")
    print(f"Next reevaluation: {profile.get('name_next_reevaluation_at', 'unknown')}")
